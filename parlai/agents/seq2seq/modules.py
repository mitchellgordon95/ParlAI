...
class HREDRNNDecoder(nn.Module):
    ...
    def forward(self, xs, encoder_output, context_vector, incremental_state=None):
        ...
        if incremental_state is not None:
            ...
            # starting fresh, or generating from scratch. Use the context vector
            # state as our start state
            #print(f"setting hiddent to context {context_vector.shape}") 
            hidden = _transpose_hidden_state(context_vector)
            #print(f"setting hiddent to context {hidden.shape}") 
        ...
        if self.attn_time != 'post':
            # no attn, we can just trust the rnn to run through
            # for HRED, looks like hidden context is already connected 
            output, new_hidden = self.rnn(xes, hidden)
        else:
            # uh oh, post attn, we need run through one at a time, and do the
            # attention modifications
            ...
            output = []
            for i in range(seqlen):
                #print(f"xes.shape is {xes.shape}") 
                #print(f"new hidden is {new_hidden.shape}") 
                o, new_hidden = self.rnn(xes[:, i, :].unsqueeze(1), new_hidden)
                o, _ = self.attention(o, new_hidden, attn_params)
                output.append(o)
            output = torch.cat(output, dim=1).to(xes.device)
